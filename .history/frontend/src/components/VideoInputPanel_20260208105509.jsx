...
import { uploadVideo, startAnalysis, getStatus, clearData } from '../utils/apiClient';
import './VideoInputPanel.css';

const VideoInputPanel = ({ onAnalysisStarted, onAnalysisComplete, onClearDetections }) => {
  ...
  const handleFileSelect = (event) => {
    ...
    if (file) {
      ...
      // Clear old detections and markers when new video is selected
      console.log('🧹 New video selected - clearing old data');
      if (onClearDetections) {
        onClearDetections();
      }
    }
  };
  ...
  const handleUploadAndAnalyze = async () => {
    ...
    try {
      // Step 0: Clear old data first
      setMessage('🧹 Clearing old data...');
      await clearData();
      console.log('✅ Old data cleared from backend');
