...
export default function PathPlanningPanel({ detections, onPathGenerated }) {
  ...
  const gridData = useMemo(() => {
    ...
    const detectionsWithGPS = detections.filter(d => d.gps);
    console.log('🔧 Total detections:', detections.length);
    console.log('🔧 Detections with GPS:', detectionsWithGPS.length);
    ...
    if (detectionsWithGPS.length === 0) {
      ...
      console.log('💡 This is likely the issue - detections need GPS');
      return null;
    }
  ...
  const handleGeneratePath = () => {
    ...
    if (!gridData) {
      ...
      alert('Cannot generate path: No detection data available');
      return;
    }
