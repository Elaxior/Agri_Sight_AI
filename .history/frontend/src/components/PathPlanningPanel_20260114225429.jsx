...
import { generateFieldGPS } from '../utils/gpsSimulator';  // ← ADD THIS IMPORT
...
export default function PathPlanningPanel({ detections, onPathGenerated }) {
  ...
  // ✅ ENRICH DETECTIONS WITH GPS (same as MapView)
  ...
  // Calculate grid and stats - USE ENRICHED DETECTIONS
  const gridData = useMemo(() => {
    console.log('🔧 [useMemo] Calculating gridData...');
    
    if (!detectionsWithGPS || detectionsWithGPS.length === 0) {
      console.log('⚠️ No detections available');
      return null;
    }

    // Check GPS coordinates
    const detectionsWithValidGPS = detectionsWithGPS.filter(d => d.gps);
    console.log('🔧 Total detections:', detectionsWithGPS.length);
    console.log('🔧 Detections with GPS:', detectionsWithValidGPS.length);
    console.log('🔧 Sample detection:', detectionsWithGPS[0]);
    console.log('🔧 Sample GPS:', detectionsWithGPS[0]?.gps);
    ...
    try {
      ...
      const grid = mapDetectionsToGrid(detectionsWithGPS);
  ...
  }, [detectionsWithGPS]);  // ← USE ENRICHED DETECTIONS
