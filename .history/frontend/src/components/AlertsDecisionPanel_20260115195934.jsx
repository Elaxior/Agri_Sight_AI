...
export default function AlertsDecisionPanel({ 
  ...
  gridStats, 
  economicImpact,
  onAlertsUpdate  // ← PART 11: NEW PROP
}) {
  ...
  useEffect(() => {
    if (!gridStats) {
      // ← PART 11: Clear alerts when no grid stats
      setAlerts([]);
      if (onAlertsUpdate) {
        onAlertsUpdate([]);
      }
    ...
    // ← PART 11: Notify parent Dashboard of alerts
    if (onAlertsUpdate) {
      onAlertsUpdate(newAlerts);
    }
  }, [detections, gridStats, economicImpact, fusionResults, onAlertsUpdate]);
