...
def clear_firebase_data(session_id=None):
    """Clear old Firebase data before new analysis."""
    try:
        ...
        if session_id:
            client.set_data(f'/sessions/{session_id}', None)
        print("🔥 Firebase data cleared")
    except Exception as e:
        print(f"⚠️ Warning: Could not clear Firebase data: {e}")
...
def run_inference_background(video_path):
    ...
    try:
        ...
        script_path = BASE_DIR / "src" / "run_inference_firebase.py"
        python_exec = "python"  # Use system python
        
        # On Windows, we need to use the proper path format
